...
float4 GetMask (InputConfig c) {
	if (c.useMask) { // assumes the mask toggle is constant, so doesn't cause a shader branching
...
float3 GetNormalTS (InputConfig c) {
	...
	if (c.useDetail) {
		...
		scale = INPUT_PROP(_DetailNormalScale) * GetMask(c).b; // sample ANySNx map only where MODS mask b is 1
